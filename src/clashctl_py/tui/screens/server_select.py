...
from clashctl_py.config import AppConfig, Server, save_config
    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            ...
            if self._cfg.servers:
                yield ListView(
                    *(
                        ListItem(Static(self._row_label(s)))
...
class ServerAddScreen(ModalScreen[Server | None]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            ...
            yield Input(placeholder="secret (leave empty if none)", id="secret", password=True)
